...
const technicalSkills = ['javascript', 'react', 'elixir', 'node', 'ruby', 'rails', 'php', 'graphql']
const softSkills = ['communication', 'problem solving', 'teamwork', 'attention to detail']
const About = () => {
    
    return (
       <div className="flex flex-col gap-4 my-4">
        <div className="gap-4 flex">
        {/* <img src={image} alt="Profile Photo" className="opacity-75 rounded-lg shadow hidden md:block max-w-sm object-contain flex-grow-0 "/> */}
        ...
        {technicalSkills.map((skill)=> {
                                            return <div className="bg-gray-200 p-2 py-1 text-sm shadow rounded w-fit">{skill}</div>
                                        })}
            </div>
