    ...
    return (
       <div className="my-4 px-10 carousel relative w-full overflow-hidden after:clear-both after:block after:content-[''] flex flex-col gap-4 justify-center items-center min-h-inherit">
        ...
        {recommendations.map((recommendation, index)=> {
                                            return (
                                                <figure class={`relative float-left w-full transition-transform duration-[600ms] ease-in-out motion-reduce:transition-none text-center ${activeRecommendation === index ? '' : 'hidden'}`}>
                                                    <FaQuoteLeft className="m-auto"/>
                                                    <blockquote cite={recommendationsLink}>
                                                        <p class="text-2xl italic font-medium text-gray-900 dark:text-white">{recommendation.content}</p>
                                                    </blockquote>
                                                <figcaption class="flex items-center justify-center mt-6 space-x-3">
                                                    ...
                                                    <div class="flex items-center divide-x-2 divide-gray-500 dark:divide-gray-700">
                                                        <p class="pr-3 font-medium text-gray-900 dark:text-white">{recommendation.owner}</p>
                                                        <p class="pl-3 text-sm text-gray-500 dark:text-gray-400">{recommendation.ownerRole}</p>
                                                    </div>
                                                    </figcaption>
                                                    </figure>)
                                        })}
